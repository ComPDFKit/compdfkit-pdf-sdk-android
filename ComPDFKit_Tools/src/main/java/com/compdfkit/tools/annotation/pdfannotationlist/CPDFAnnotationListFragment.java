...
public class CPDFAnnotationListFragment extends CBasicThemeFragment {
     ...
     */
    private ActivityResultLauncher<Intent> importAnnotFileLauncher = registerForActivityResult(
        new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == Activity.RESULT_OK) {
                ...
                if (fileName.toLowerCase().endsWith(".xfdf")) {
                    ...
                    String dir = new File(getContext().getCacheDir(),
                        CFileUtils.CACHE_FOLDER + File.separator + "xfdfFile").getAbsolutePath();
                    // Get the saved file path
                    String importFilePath = CFileUtils.copyFileToInternalDirectory(getContext(),
                        uri, dir, fileName);
                    CLog.e("ComPDFKit-Tools", "importFilePath:" + importFilePath);

                    if (!TextUtils.isEmpty(importFilePath)) {
                        boolean importResult = CPDFAnnotDatas.importAnnotations(
                            pdfView.getCPdfReaderView().getPDFDocument(), importFilePath);
                        if (importResult) {
                        ...
                        CLog.e("ComPDFKit-Tools",
                            "import Annotation xfdf format file " + (importResult ? "success"
                                : "fail"));
                    }
                } else {
                    CLog.e("ComPDFKit-Tools", "Please select xfdf format file");
                    CToastUtil.showLongToast(getContext(),
                        R.string.tools_please_select_xfdf_format_file);
                }
            }
        });
            ...
            for (CPDFBotaGlobalMenuItem cpdfBotaGlobalMenuItem : annotationMenu.getGlobal()) {
                switch (cpdfBotaGlobalMenuItem.getId()) {
                    case CPDFBotaAnnotationMenu.GLOBAL_MENU_ID_IMPORT_ANNOTATION:
                        menuWindow.addItem(R.string.tools_import_annotations,
                            v -> importAnnotFileLauncher.launch(
                                CFileUtils.getIntent("application/octet-stream")));
                        break;
                    case CPDFBotaAnnotationMenu.GLOBAL_MENU_ID_EXPORT_ANNOTATION:
                        menuWindow.addItem(R.string.tools_export_annotations, hasAnnotations, v -> {
                            ...
                            checkExportAnnotationsStoragePermissions();
                        });
                        break;
                    case CPDFBotaAnnotationMenu.GLOBAL_MENU_ID_REMOVE_ALL_ANNOTATION:
                        menuWindow.addItem(R.string.tools_delete_all_annotations, hasAnnotations,
                            v -> deleteAllAnnotations());
                        break;
    ...
    private void checkExportAnnotationsStoragePermissions() {
        showExportAnnotationDirectoryDialog();
    }

    private void showExportAnnotationDirectoryDialog(){
        ...
        CFileDirectoryDialog directoryDialog = CFileDirectoryDialog.newInstance(
            dirPath, getString(R.string.tools_saving_path),
            getString(R.string.tools_okay));
        directoryDialog.setSelectFolderListener(dir -> {
            CPDFDocument document = pdfView.getCPdfReaderView()
                .getPDFDocument();
            boolean result = CPDFAnnotDatas.exportAnnotations(document, dir,
                CFileUtils.getFileNameNoExtension(document.getFileName()));
            if (result) {
                CToastUtil.showLongToast(getContext(),
                    R.string.tools_export_success);
            }
        });
    ...
    private void showAnnotationMoreMenu(CPDFAnnotListItem item, int position, View anchorView) {
                    ...
                    case CPDFBotaAnnotationMenu.MORE_MENU_ID_ADD_REPLY:
                        moreMenu.addItem(R.string.tools_add_a_new_reply, v -> {
                            ...
                            editReplyDialogFragment.setReplyContentListener(content -> {
                                CPDFReplyAnnotation replyAnnotation = item.getAttr()
                                    .createReplyAnnotation();
                                replyAnnotation.setTitle(
                            ...
                            editReplyDialogFragment.show(getChildFragmentManager(),
                                "addReplyDialogFragment");
                        });
                        break;
                    case CPDFBotaAnnotationMenu.MORE_MENU_ID_VIEW_REPLY:
                        moreMenu.addItem(R.string.tools_view_replies,
        ...
        moreMenu.showAtLocation(anchorView, Gravity.START | Gravity.TOP, windowPos[0],
            windowPos[1]);
    }
    ...
    public void setPDFDisplayPageIndexListener(
        COnSetPDFDisplayPageIndexListener displayPageIndexListener) {
